# Other animal functions for ShelterLuv sync
# Developed for Dallas Pets Alive by Katie Patterson www.kirska.com
import collections
from Local_Defines import OTHER_LIST_FILE, PLACEHOLDER_IMAGE, LIST_THEME_PATH, PET_LINK_RELATIVE_PATH
...
def parse_other(animals_in):
    other_list = collections.OrderedDict()

    # get the cats
    for animal in animals_in:
        if animals_in[animal]["Type"] != "Cat" and animals_in[animal]["Type"] != "Dog":
            other_list[animal] = animals_in[animal]

    generate_other_list(other_list)
...
def parse_other_profile(animal):
    ...
    output += "<b>Type: </b>"
    output += animal["Type"].encode('utf-8')
    output += "<br/>\n"
